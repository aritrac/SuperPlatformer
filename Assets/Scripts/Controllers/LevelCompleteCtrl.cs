...
using UnityEngine;
using UnityEngine.UI;                                           //provides access to Unity AI
...
public class LevelCompleteCtrl : MonoBehaviour {
    ...
    public Sprite golderStar;                                   //awarded when score is above a certain value
    ...
    public int ScoreForThreeStars;                              //score required for earning 3 golden stars
    public int ScoreForTwoStars;                                //score required for earning 2 golden stars
    public int ScoreForOneStar;                                 //score required for earning 1 golden star
    public int ScoreForNextLevel;                               //score required for unlocking the next level
    ...
    IEnumerator HandleFirstStarAnim(Image starImg)
    {
        ...
        if (showTwoStars || showThreeStars)
            ...
            Invoke("CheckLevelStatus", 1.2f);                   //for enabling the next level button, if required number of stars are attained for the level
    }
